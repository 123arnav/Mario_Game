...
function play(){
  game_status="loading";
  document.getElementById("status").innerHTML="Status: "+game_status;
};
function game(){
  ...
  if(gameConfig.status==='start' && game_status=="loading"){
...
function changeGameStatud(character){
  if(game_status="loading" && noseX!="" && gameConfig.status==="start") {
    world_start.play();
